...
export class IndexerService {
  ...
  async poll() {
    ...
    if (this.currentEvent) {
      this.currentEvent.maxLossBps = Math.max(this.currentEvent.maxLossBps, sample.lossQuoteBps);
      this.currentEvent.minRBps = Math.min(this.currentEvent.minRBps, sample.rBps);
    ...
    const event = this.detector.sample(Math.floor(sample.ts.getTime() / 1000), sample.rBps);
    if (!event) {
      return;
    }
    ...
    if (event.type === 'DEPEG_START') {
      ...
      logger.info({ riskId, snapshotCid, rBps: sample.rBps }, 'depeg_window_opened');
      return;
    }

    if (event.type === 'DEPEG_END' && this.currentEvent) {
      const severity = Math.max(this.currentEvent.maxLossBps, sample.lossQuoteBps);
      ...
      logger.info(
        { riskId: this.currentEvent.riskId, snapshotCid, duration: event.end - this.currentEvent.start },
        'depeg_window_closed',
      );
      ...
      values: [
        {
          ...
          window_end: params.windowEnd ? toDateTimeString(params.windowEnd * 1000) : null,
          severity_bps: params.severityBps,
          twap_bps: params.twapBps,
          r_bps: params.rBps,
          attested_at: toDateTimeString(now),
          attestor: env.SIGNER_PRIVATE_KEY ? deriveAddress(env.SIGNER_PRIVATE_KEY) : '0x0000000000000000000000000000000000000000',
          snapshot_cid: '',
